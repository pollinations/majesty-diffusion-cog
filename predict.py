...
from itertools import prodself.uct
...
class Predictor(BasePredictor):
    def setup(self):
        ...
        config = OmegaConf.load("./latent-diffusion/configs/latent-diffusion/txt2img-1p4B-eval.yaml")  # TODO: self.optionally download from same location as ckpt and chnage this logic
        model = load_model_from_config(config, f"{model_path}/latent_diffusion_txt2img_f8_large.ckpt",False, "finetuned")  # TODO: check path
        self.model = model.half().eval().to(device)

        #Alstro's aesthetic model
        self.aesthetic_model_336 = torch.nn.Linear(768,1).cuda()
        self.aesthetic_model_336.load_state_dict(torch.load(f"{model_path}/ava_vit_l_14_336_linear.pth"))

        self.aesthetic_model_224 = torch.nn.Linear(768,1).cuda()
        self.aesthetic_model_224.load_state_dict(torch.load(f"{model_path}/ava_vit_l_14_linear.pth"))

        self.aesthetic_model_16 = torch.nn.Linear(512,1).cuda()
        self.aesthetic_model_16.load_state_dict(torch.load(f"{model_path}/ava_vit_b_16_linear.pth"))

        self.aesthetic_model_32 = torch.nn.Linear(512,1).cuda()
        self.aesthetic_model_32.load_state_dict(torch.load(f"{model_path}/sa_0_4_vit_b_32_linear.pth"))
        ...
        self.cut_ic_pow = .2
        self.cut_icgray_p = [.1]*300+[0]*1000
        self.cutn_batches = 1
        ...
        self.active_function = "softsign" # function to manipulate the gradient - help things to stablize
    ...
    def unpurge(self):
        self.clip_model, self.clip_size, self.self.clip_tokenize, self.self.clip_normalize, self.clip_list = self.full_clip_load(clip_load_list)
        self.has_purged = False
    
    def load_clip_models(self, mmc_models):
        clip_model, clip_size, self.clip_tokenize, self.clip_normalize= {},{},{},{}
        clip_list = []
        for item in mmc_models:
            ...
            for model_loader in model_loaders:
                ...
                clip_size[item["id"]] = self.clip_model[item["id"]].visual.input_resolution
                self.clip_tokenize[item["id"]] = self.clip_model[item["id"]].preprocess_text()
                self.clip_normalize[item["id"]] = normalize
        return clip_model, clip_size, self.clip_tokenize, self.clip_normalize, clip_list
    ...
    def full_clip_load(self, clip_load_list):
        ...
        gc.collect()
        try:
            del self.clip_model, self.clip_size, self.clip_tokenize, self.clip_normalize, self.clip_list
        except:
            pass
        mmc_models = get_mmc_models(clip_load_list)
        self.clip_model, self.clip_size, self.clip_tokenize, self.clip_normalize, self.clip_list = self.load_clip_models(mmc_models)
        return self.clip_model, self.clip_size, self.clip_tokenize, self.clip_normalize, self.clip_list

    
    def predict(self,
        text_prompt: str = cog.Input(description="Your text prompt.", default=""),
        latent_diffusion_model: str =  cog.Input(description='Original is the previous LAION model. Finetuned should be better but cannot do text. One of:  ["original", "finetuned", "ongo (fine tuned in paintings)", "erlich (fine tuned in logos)"]', default='finetuned'), #@param ["original", "finetuned", "ongo (fine tuned in paintings)", "erlich (fine tuned in logos)"]
        latent_diffusion_guidance_scale: int = cog.Input(description="Balance between creativity and coherent composition. Try values between 0-15. Lower values help with text interpretation and creativity, higher help with composition. ", default=12),
    ):
        ...
        for n in trange(how_many_batches, desc="Sampling"):
            ...
            if self.opt.W != width or self.opt.H != height:
                print(f'Changing output size to {self.opt.W}x{self.opt.H}. Dimensions must by multiples of 64.')
        ...
        for i in self.clip_list:
             make_cutouts[i] = MakeCutouts(self.clip_size[i][0] if type(self.clip_size[i]) is tuple else self.clip_size[i],Overview=1)
            ...
            for i in self.clip_list:
                if "cloob" not in i:
                    with torch.cuda.amp.autocast():
                        embeds = self.clip_model[i].encode_text(self.clip_tokenize[i](txt).to(device))
        ...
        for prompt in image_prompts:
            ...
            img = TF.resize(img, min(self.opt.W, self.opt.H, *img.size), transforms.InterpolationMode.LANCZOS)
            for i in self.clip_list:
                if "cloob" not in i:
                    with torch.cuda.amp.autocast():
                        batch = make_cutouts[i](TF.to_tensor(img).unsqueeze(0).to(device))
                        embed = self.clip_model[i].encode_image(self.clip_normalize[i](batch))
        ...
        if self.init_image is not None:
            ...
            if self.init_rotate: self.init = torch.rot90(self.init, 1, [3,2]) 
            x0_original = torch.tensor(self.init)
            self.init = resize(self.init,out_shape = [self.opt.n_samples,3,self.opt.H, self.opt.W])
            self.init = self.init.mul(2).sub(1).half()
            self.init_encoded =  self.model.first_stage_model.encode(self.init).sample()* self.init_magnitude + self.init_brightness
        ...
        if self.init_mask is not None:
            ...
            if self.mask_rotate: mask = torch.rot90(mask, 1, [3,2])
            ...
            with torch.cuda.amp.autocast():
                with self.model.ema_scope():
                    ...
                    if self.opt.scale != 1.0:
                        self.uc = self.model.get_learned_conditioning(self.opt.n_samples * self.opt.self.uc).cuda()
                    
                    for n in range(self.opt.n_iter):
                        torch.cuda.empty_cache()
                        gc.collect()
                        self.c = self.model.get_learned_conditioning(self.opt.n_samples * prompt).cuda()
                        if self.init_encoded is None:
                            x_T = torch.randn([self.opt.n_samples,*shape], device=device)
                            upscaled_flag = False
                            x0 = None
                        else:
                        ...
                        for custom_schedule in custom_schedules:
                            if type(custom_schedule) != type(""):
                                ...
                                samples_ddim, _ = sampler.sample(S=self.opt.ddim_steps,
                                                                 ...
                                                                 eta_end=eta2,
                                                                 img_callback=None if self.use_cond_fn else self.display_handler,
                                                                 ...
                                                                 clip_embed = self.target_embeds["ViT-L-14--openai"] if "ViT-L-14--openai" in self.clip_list else None
                                                                )
                                ...
                                if method == "RGB":
                                    ...
                                    self.init = Image.open(fetch(temp_file)).convert('RGB')
                                    self.init = TF.to_tensor(self.init).to(device).unsqueeze(0)
                                    self.opt.H, self.opt.W = self.opt.H*self.scale_factor, self.opt.W*self.scale_factor
                                    self.init = resize(self.init,out_shape = [self.opt.n_samples,3,self.opt.H, self.opt.W], antialiasing=True)
                                    self.init = self.init.mul(2).sub(1).half()
                                    x_T =  (self.model.first_stage_model.encode(self.init).sample()*self.init_magnitude)
                                    upscaled_flag = True
                                ...
                                if method == "gfpgan":
                                    ...
                                    torch.cuda.empty_cache()
                                    gc.collect()
                                    os.system(f"cd GFPGAN && python inference_gfpgan.py -i {temp_file} -o results -v {GFP_ver} -s {GFP_factor}")
                                    face_corrected = Image.open(fetch(f"GFPGAN/results/restored_imgs/{temp_file_name}"))
                                    with io.BytesIO() as output:
                                      face_corrected.save(output,format="PNG")
                                    self.init = Image.open(fetch(f"GFPGAN/results/restored_imgs/{temp_file_name}")).convert('RGB')
                                    self.init = TF.to_tensor(self.init).to(device).unsqueeze(0)
                                    self.opt.H, self.opt.W = self.opt.H*self.scale_factor, self.opt.W*self.scale_factor
                                    self.init = resize(self.init,out_shape = [self.opt.n_samples,3,self.opt.H, self.opt.W], antialiasing=True)
                                    self.init = self.init.mul(2).sub(1).half()
                                ...
                                if method =="noise":
                                    self.scale_factor = float(self.scale_factor)
                                    x_T += noise_like(x_T.shape,device,False)*self.scale_factor
                                if method == "purge":
                                    ...
                                    for i in self.scale_factor.split(","):
                                        if i in clip_load_list:
                                            ...
                                            print("Purge ",i)
                                            del self.clip_list[self.clip_list.index(m_id)]
                                            del self.clip_model[m_id]
                                            del self.clip_size[m_id]
                                            del self.self.clip_tokenize[m_id]
                                            del self.self.clip_normalize[m_id]
                        ...
                        if(last_step_upscale and method == 'gfpgan'):
                          ...
                          Image.fromarray(self.image_grid.astype(np.uint8)).save(os.path.join(outpath, f'{current_time}.png'), format = "PNG")
        ...
        if(len(all_samples) > 1):
            ...
            Image.fromarray(grid.astype(np.uint8)).save(os.path.join(outputs_path, f'output.png'))
    ...
    def cond_fn(self, x, t):
        ...
        with torch.enable_grad():
            ...
            for i in self.clip_list:
                make_cutouts[i] = MakeCutouts(self.clip_size[i][0] if type(self.clip_size[i]) is tuple else self.clip_size[i],
            ...
            for j in range(self.cutn_batches):
                losses=0
                for i in self.clip_list:
                    clip_in = self.self.clip_normalize[i](make_cutouts[i](x_in.add(1).div(2)).to("cuda"))
                    image_embeds = self.clip_model[i].encode_image(clip_in).float().unsqueeze(0).expand([self.target_embeds[i].shape[0],-1,-1])
                    self.target_embeds_temp = self.target_embeds[i]
                    if i == 'ViT-B-32--openai' and self.experimental_aesthetic_embeddings:
                        aesthetic_embedding = torch.from_numpy(np.load(f'aesthetic-predictor/vit_b_32_embeddings/rating{self.experimental_aesthetic_embeddings_score}.npy')).to(device) 
                        aesthetic_query = self.target_embeds_temp + aesthetic_embedding * self.experimental_aesthetic_embeddings_weight
                        self.target_embeds_temp = (aesthetic_query) / torch.linalg.norm(aesthetic_query)
                    if i == 'ViT-L-14--openai' and self.experimental_aesthetic_embeddings:
                        aesthetic_embedding = torch.from_numpy(np.load(f'aesthetic-predictor/vit_l_14_embeddings/rating{self.experimental_aesthetic_embeddings_score}.npy')).to(device) 
                        aesthetic_query = self.target_embeds_temp + aesthetic_embedding * self.experimental_aesthetic_embeddings_weight
                        self.target_embeds_temp = (aesthetic_query) / torch.linalg.norm(aesthetic_query)
                    self.target_embeds_temp = self.target_embeds_temp.unsqueeze(1).expand([-1,cutn*n,-1])          
                    ...
                    if i == "ViT-L-14-336--openai" and self.aes_scale !=0:
                        aes_loss = (self.aesthetic_model_336(F.normalize(image_embeds, dim=-1))).mean() 
                        losses -= aes_loss * self.aes_scale 
                    if i == "ViT-L-14--openai" and self.aes_scale !=0:
                        aes_loss = (self.aesthetic_model_224(F.normalize(image_embeds, dim=-1))).mean() 
                        losses -= aes_loss * self.aes_scale 
                    if i == "ViT-B-16--openai" and self.aes_scale !=0:
                        aes_loss = (self.aesthetic_model_16(F.normalize(image_embeds, dim=-1))).mean() 
                        losses -= aes_loss * self.aes_scale  
                    if i == "ViT-B-32--openai" and self.aes_scale !=0:
                        aes_loss = (self.aesthetic_model_32(F.normalize(image_embeds, dim=-1))).mean()
                        losses -= aes_loss * self.aes_scale
            ...
            loss =  losses
            #del losses
            if self.symmetric_loss_scale != 0: loss +=  symmetric_loss(x_in) * self.symmetric_loss_scale
            if self.init_image is not None and self.init_image:
                lpips_loss = (lpips_model(x_in, self.init) * self.init_scale).squeeze().mean()
                #print(lpips_loss)
                loss += lpips_loss
            range_scale= self.range_index[t]
            range_losses = range_loss(x_in,self.RGB_min,self.RGB_max).sum() * range_scale
            loss += range_losses
            ...
            if self.grad_center: grad = centralized_grad(grad, use_gc=True, gc_conv_only=False)
            ...
            if t>=0:
                ...
                if self.active_function=="clamp":
                    grad = grad.clamp(-mag*self.grad_scale*2,mag*self.grad_scale*2)
            if grad.abs().max()>0:
            ...
            mean_losses = (x.mean().abs() - self.mean_range).abs().clamp(min = 0)*mean_scale
            tv_losses = tv_loss(x).sum() * self.tv_scales[0] +\
                tv_loss(F.interpolate(x, scale_factor= 1/2)).sum()* self.tv_scales[1] + \
                tv_loss(F.interpolate(x, scale_factor = 1/4)).sum()* self.tv_scales[2] + \
                tv_loss(F.interpolate(x, scale_factor = 1/8)).sum()* self.tv_scales[3] 
    ...
    def grad_fn(self, x,t):
        if t <= 500 and self.grad_blur: x = TF.gaussian_blur(x, 2*round(int(max(self.grad_blur-t/150, 1)))-1, 1.5)
        return x
    ...
    def cond_clamp(self, image,t): 
        ...
        if t<=self.punish_steps:
            image = image.clamp(min = -ths, max =  ths)+(im_max-im_min) * self.punish_factor  #((im_max-im_min)*punish_factor).tanh()/punish_factor 
        if t<= self.compress_steps:
            image = image / (ths/self.threshold)**self.compress_factor
            image +=  noise_like(image.shape,device,False) * ((ths/self.threshold)**self.compress_factor - 1)
        return(image)
